...
def connect_corners_uv(uv1: np.ndarray, uv2: np.ndarray, length=256) -> np.ndarray:
    ...
    u_unique, u_indices = np.unique(p_u1, return_inverse=True)
    # 对重复的u值进行处理
    for u in u_unique:
        # 获取该u值在原始数组中的索引
        indices = np.where(p_u1 == u)[0]
        # 如果索引数量大于1，说明有重复的u值
        if len(indices) > 1:
            for i in indices[1:]:
                # 将重复u值对应的v值逐个加1
    ...
    # elif abs(p_u1 - p_u2) < 2:
    #     start = np.floor(min(p_u1, p_u2))
    #     p = max(p_u1, p_u2)
    #     end = np.ceil(p)
    #     if end == np.ceil(p):
    #         end = end - 1
...
def visibility_corners(corners):
    plan_y = boundary_type(corners)#判断是天花板还是地面
    xyz = uv2xyz(corners, plan_y)
    xz = xyz[:, ::2]
    #计算在xz平面上的可见多边形，确定哪些点属于同一个平面。
    xz = calc_visible_polygon(center=np.array([0, 0]), polygon=xz, show=False) #确定那两个点属于一个平面
    xyz = np.insert(xz, 1, plan_y, axis=1) #在第一维插入y
    output = xyz2uv(xyz).astype(np.float32)#转成uv
    return output
...
def corners2boundary(corners: np.ndarray, step=0.01, length=None, visible=True) -> np.ndarray:
    ...
    for j in range(n_con):
        # 链接当前角点和下一个角点，生成边界点坐标
        uv = connect_corners(corners[j], corners[(j + 1) % n_con], step, length)
        # if corners[j, 0] - corners[j + 1, 0] < 0.001:
        #     corners[j + 1, 0] = corners[j + 1, 0] + 0.002
        if uv is None:
            continue
        if uv.shape[0] == 0:
            # uv = np.concatenate((corners[j], corners[j+1]))
            corners[j + 1, 0] = corners[j + 1, 0] + 0.002
            uv = connect_corners(corners[j], corners[(j + 1) % n_con], step, length)
        if boundary is None:
            boundary = uv
    ...
    # 使数组的起始点位于最小的x坐标
    boundary = np.roll(boundary, -boundary.argmin(axis=0)[0], axis=0)  # 256 2
...
def corners2boundaries1(ceil_uv: np.ndarray = None,
                       floor_uv: np.ndarray = None,step=0.01,
