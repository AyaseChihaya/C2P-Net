...
def calculate_end_point_angles(light_source: Point, segment: Segment) -> None:
    x = light_source.x
    y = light_source.y
    dx = 0.5 * (segment.p1.x + segment.p2.x) - x  #求中心点x
    dy = 0.5 * (segment.p1.y + segment.p2.y) - y  #求中心点y
    segment.d = (dx * dx) + (dy * dy)   #到原点的距离
    segment.p1.angle = math.atan2(segment.p1.y - y, segment.p1.x - x)  #第一个点对于原点的角度
    segment.p2.angle = math.atan2(segment.p2.y - y, segment.p2.x - x)  #第二个点对于原点的角度


def set_segment_beginning(segment: Segment) -> None:
    d_angle = segment.p2.angle - segment.p1.angle #判断两个角度
    if d_angle <= -math.pi:
        d_angle += 2 * math.pi
    if d_angle > math.pi:
        d_angle -= 2 * math.pi  #放到-3.14    3.14
...
def polygon_to_segments(polygon: np.array) -> np.array:
    segments = []
    polygon = np.concatenate((polygon, [polygon[0]]))#把第一个点放在最后一个点后面
    for i in range(len(polygon) - 1):#循环遍历所有点
        p1 = polygon[i]
        p2 = polygon[i + 1]
        segments.append([p1, p2])#两个点之间构成一个平面
...
def calc_visible_polygon(center: np.array, polygon: np.array = None, segments: np.array = None, show: bool = False):
    if segments is None and polygon is not None:#polygon多边形的顶点坐标数组
        #segments线段的起始点和终点坐标数组
        #将多边形转换为线段
        segments = polygon_to_segments(polygon) #每两个点构成平面

    origin = Point(x=center[0], y=center[1]) #中心点的二维坐标数组
    endpoints = []
    for s in segments:#遍历各个线段 ，计算每个线段与中心点的距离和角度
        p1 = s[0]
        p2 = s[1]
        segment = Segment(x1=p1[0], y1=p1[1], x2=p2[0], y2=p2[1])
        calculate_end_point_angles(origin, segment)  #计算距离和角度
        set_segment_beginning(segment)
        endpoints.extend([segment.p1, segment.p2])  #端点

    open_segments = []
    output = []
    begin_angle = 0 #起始角度 begin_angle 为 0
    endpoints = sorted(endpoints, key=ctk(endpoint_compare)) #排序

    for pas in range(2):#通过两次循环处理 endpoints 中的每个端点
#如果端点是线段的起始点，则将该线段添加到 open_segments 中，
#如果端点不是起始点且该线段在 open_segments 中，则将该线段从 open_segments 中移除
#当遇到新的 open_segment时，根据起始角度和当前端点的角度，计算三角形的顶点坐标，并将其添加到 output 列表中
    ...
    output_polygon = []#存储最终的多边形顶点坐标
    # Remove duplicate删除重复项
    for i, p in enumerate(output):#遍历 output 列表中的点
        q = output[(i + 1) % len(output)]
        if int(p.x * 10000) == int(q.x * 10000) and int(p.y * 10000) == int(q.y * 10000):
            continue#根据坐标值判断是否为重复点
