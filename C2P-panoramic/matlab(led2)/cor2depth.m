...
data_path = '../predict_param/param_(1-27)train/';
...
%     disp(i);
    ...
    loc_center_map1 = squeeze(center_map(1,1,:,:));%预测的1通道为角点坐标
    loc_center_map2 = double(sigmoid(loc_center_map1));
    loc_center_map3 = gaussian_thresholding_nms(loc_center_map2, 0.25, 30);
    h = ones(9);
    b = ordfilt2(loc_center_map3,81,h);
    loc_center_map = double(loc_center_map3 > 0);
    [index_inn_cory, index_inn_corx] = find(loc_center_map==1);
    pixel_corner = [index_inn_corx,index_inn_cory]; %zhongxin点坐标
%     pixel_corner(:,2) = pixel_corner(:,2) -2;

    loc_corner_map1 = squeeze(corner_map(1,1,:,:));%预测的1通道为角点坐标
    loc_corner_map2 = double(sigmoid(loc_corner_map1));
    loc_corner_map3 = gaussian_thresholding_nms(loc_corner_map2, 0.13, 5);
    h1 = ones(9);
    b1 = ordfilt2(loc_corner_map3,81,h);
    loc_corner_map = double(loc_corner_map3 > 0);
    [index_inn_cory1, index_inn_corx1] = find(loc_corner_map==1);
    pixel_corner1 = [index_inn_corx1,index_inn_cory1]; %角点坐标

    loc_normal_map1 = squeeze(normal_pred(1,:,:,:));
    normal = zeros(length(pixel_corner) ,2);
    for n = 1:length(pixel_corner)
        normal(n,:) = loc_normal_map1(:,index_inn_cory(n),index_inn_corx(n));
%         radius = 5; % 圆的半径  
%         %确保索引位置在合法范围内
...
%         normal(n,:) = [a_mean c_mean];
    ...
    xyz_corner = zeros(length(pixel_corner1) ,3);
    for ii = 1:length(xyz_corner) 
        xyz_corner(ii,:) = XY2xyz(pixel_corner1(ii,:));
%         xyz_center(ii,:) = trans_eval(pixel_corner(ii,:));
    end
    xyz = [xyz_corner(:,1) xyz_corner(:,3) xyz_corner(:,2)];
    [~,~,p] = affine_fit(xyz(1:2:end,:));%用上面四个点计算最适合的一组天花板采样点平面(该平面法线距离的最小平方)
    % 地面
    [~,~,p2] = affine_fit(xyz(2:2:end,:));%用下面四个点计算最适合的一组地面采样点平面(该平面法线距离的最小平方)

    xyz_center = zeros(length(pixel_corner) ,3);
    for ii = 1:length(xyz_center) 
        xyz_center(ii,:) = XY2xyz(pixel_corner(ii,:));
%         xyz_center(ii,:) = trans_eval(pixel_corner(ii,:));
    end
    xyz_center = [xyz_center(:,1) xyz_center(:,3) xyz_center(:,2)];
    [~,~,p1] = affine_fit(xyz_center);%天花板的一个点
    ceil = [p1(:,1) p1(:,2) p1(:,3)+room_height/2];
    floor = [p1(:,1) p1(:,2) p1(:,3)-room_height/2]; 
    xyz_all = [p;p2;xyz_center];
    ...
    for j = 1:size(xyz_im,1)%循环每个像素点，用（0,0,0）和每个像素点构成的线段
        if mod(size(xyz_center,1),2)~=0
            check = zeros(1+(1+size(xyz_center,1))/2,1);
            I = zeros(1+(1+size(xyz_center,1))/2,3);
        end
        if mod(size(xyz_center,1),2)==0
            check = zeros(2+size(xyz_center,1)/2,1);
            I = zeros(2+size(xyz_center,1)/2,3);
        end
