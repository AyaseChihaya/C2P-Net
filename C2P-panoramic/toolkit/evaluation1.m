function [meaniou_2d,meaniou_3d,mean_rmse,mean_delta1] = evaluation1( result, gt_result,sz )
GlobalParameters;
h=128;
w=256;
...
for i = 1:length(result)
    ...
    value = max(pred_seg(:));
    value = value(1);
    pred_seg(pred_seg<value-1)=3;
    pred_seg(pred_seg==value)=2;
    pred_seg(pred_seg==value-1)=1;
...
%     mm = mean(threshold(:));
    delta1_val = mean(threshold<1.7);%1.09    %1.2
%     Are =  abs((pred_depth-gt_depth)./gt_depth);
    

end
% meanPxError = mean(allPxError);
meaniou_2d = mean(iou_2d);
