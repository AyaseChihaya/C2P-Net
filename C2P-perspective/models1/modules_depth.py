...
class R1(nn.Module):
    def __init__(self, block_channel):
        ...
        self.conv0 = nn.Conv2d(2048, 2048, (3, 3), stride=1, padding=0)
        self.conv1 = nn.Conv2d(2048, 2048, (6, 8), stride=1, padding=0)
        self.conv2 = nn.Conv2d(2048, 512, (1, 1), stride=1, padding=0)
        self.conv3 = nn.Conv2d(512, 4, (1, 1), stride=1, padding=0)
...
class D1(nn.Module):
    ...
    def __init__(self, num_features = 2048):
        ...
        self.conv = nn.Conv2d(num_features+512, num_features //
                               2, kernel_size=1, stride=1, bias=False)
        num_features = num_features // 2
        self.bn = nn.BatchNorm2d(num_features)
        ...
        self.t0 = nn.ConvTranspose2d(2048, 512, (4, 5), stride=1, padding=0)
        self.t1 = nn.Conv2d(512, 512, (3, 3), stride=1, padding=1)

    def forward(self, x_block1, x_block2, x_block3, x_block4):

        x0 = F.relu(self.conv0(x_block4))
        x1 = F.relu(self.conv1(x0))
        x2 = F.relu(self.conv2(x1))
        x3 = self.conv3(x2)

        x_t0 = F.relu(self.t0(x1))
        x_t0 = F.upsample(x_t0, scale_factor=2, mode='bilinear')
        x_t1 = F.relu(self.t1(x_t0))

        x_cat = torch.cat((x_block4, x_t1), 1)
        x_d0 = F.relu(self.bn(self.conv(x_cat)))
        x_d1 = self.up1(x_d0, [x_block3.size(2), x_block3.size(3)])
        x_d2 = self.up2(x_d1, [x_block2.size(2), x_block2.size(3)])
        x_d3 = self.up3(x_d2, [x_block1.size(2), x_block1.size(3)])
        x_d4 = self.up4(x_d3, [x_block1.size(2)*2, x_block1.size(3)*2])
        out = self.refine(x_d4)
