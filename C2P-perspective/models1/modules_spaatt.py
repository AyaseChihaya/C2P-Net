...
class R_MASK(nn.Module):
    def __init__(self, block_channel):
        ...
        self.conv3 = nn.Conv2d(num_features, 32,
                               kernel_size=3, stride=1, padding=1, bias=True)
        # self.gn3 = nn.GroupNorm(8, num_features)
        # self.bn3 = nn.BatchNorm2d(num_features)
        #
...
class R_PLANE(nn.Module):
    def __init__(self, block_channel):
        ...
        self.convatt = nn.Conv2d(2, 1, kernel_size=7, stride=1, padding=3)
        # self.gn3 = nn.GroupNorm(8, num_features)
        # self.bn3 = nn.BatchNorm2d(num_features)
        #
        # self.conv4 = nn.Conv2d(num_features, num_features,
    ...
    def forward(self, x):
        ...
        out = torch.sigmoid(self.convatt(out))

        x0 = F.relu(self.bn0(self.conv0(x * out)))
        x1 = F.relu(self.bn1(self.conv1(x0)))
        x2 = F.relu(self.bn2(self.conv2(x1)))
        # x3 = F.relu(self.bn3(self.conv3(x2)))
        # x4 = F.relu(self.bn4(self.conv4(x3)))
