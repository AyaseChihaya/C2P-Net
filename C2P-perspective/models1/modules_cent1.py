...
class R_PARAM(nn.Module):
    def __init__(self, block_channel):
        ...
        self.conv3 = nn.Conv2d(num_features, 5,
                               kernel_size=3, stride=1, padding=1, bias=True)
...
class R_LOC(nn.Module):
    def __init__(self, block_channel):
        ...
        self.convd0 = nn.Conv2d(2048, num_features * 2,
                               kernel_size=1, stride=1, padding=0, bias=True)
        self.bnd0 = nn.BatchNorm2d(num_features * 2)

        self.up1 = _UpProjection(
            num_input_features=num_features * 2, num_output_features=num_features)
    ...
    def forward(self, x_block3, x_block4):

        x_d0 = F.relu(self.bnd0(self.convd0(x_block4)))
        x_d1 = self.up1(x_d0, [x_block3.size(2), x_block3.size(3)])
        x0 = F.relu(self.bn0(self.conv0(x_d1)))
        x1 = F.relu(self.bn1(self.conv1(x0)))
        x2 = F.relu(self.bn2(self.conv2(x1)))
...
class R_MASK(nn.Module):
    def __init__(self, block_channel):
        ...
        self.conv3 = nn.Conv2d(num_features, 28,
                               kernel_size=3, stride=1, padding=1, bias=True)
        # self.gn3 = nn.GroupNorm(8, num_features)
        # self.bn3 = nn.BatchNorm2d(num_features)
        #
...
class D_PLANE(nn.Module):

    def __init__(self, num_features = 2048):
        super(D_PLANE, self).__init__()
        self.conv = nn.Conv2d(num_features, num_features //
                               2, kernel_size=1, stride=1, bias=False)
        num_features = num_features // 2
        self.bn = nn.BatchNorm2d(num_features)
...
class R_PLANE(nn.Module):
    def __init__(self, block_channel):
        ...
        self.conv3 = nn.Conv2d(num_features, 2,
                               kernel_size=3, stride=1, padding=1, bias=True)
        # self.gn3 = nn.GroupNorm(8, num_features)
        # self.bn3 = nn.BatchNorm2d(num_features)
        #
