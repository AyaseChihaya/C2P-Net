...
# import resnet
# import densenet
from models import modules_spaatt, resnet, densenet, senet
...
class model(nn.Module):
    def __init__(self, Encoder, num_features, block_channel):
        ...
        self.E = Encoder
        self.D = modules_spaatt.D(num_features)
        self.MFF = modules_spaatt.MFF(block_channel)
        self.R = modules_spaatt.R(block_channel)

        self.C = modules_spaatt.C(block_channel)
        self.R_LOC = modules_spaatt.R_LOC(block_channel)

        self.D_MASK = modules_spaatt.D_MASK(num_features)
        self.R_MASK = modules_spaatt.R_MASK(block_channel)

        self.D_PLANE = modules_spaatt.D_PLANE(num_features)
        self.R_PLANE = modules_spaatt.R_PLANE(block_channel)
