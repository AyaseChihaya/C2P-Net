...
# import resnet
# import densenet
from models import modules_cent1, resnet, densenet, senet
...
class model(nn.Module):
    def __init__(self, Encoder, num_features, block_channel):
        ...
        self.E = Encoder
        self.D = modules_cent1.D(num_features)
        self.MFF = modules_cent1.MFF(block_channel)
        # self.R = modules_cent1.R(block_channel)
        self.R_PARAM = modules_cent1.R_PARAM(block_channel)

        self.R_LOC = modules_cent1.R_LOC(block_channel)

        self.D_MASK = modules_cent1.D_MASK(num_features)
        self.R_MASK = modules_cent1.R_MASK(block_channel)

        # self.D_PLANE = modules_cent1.D_PLANE(num_features)
        # self.R_PLANE = modules_cent1.R_PLANE(block_channel)
    ...
    def forward(self, x, coord_feat):
        ...
        # out_ivd = self.R(torch.cat((x_decoder, x_mff), 1))
