...
for j = 1:size(combs,1)
    ...
    if ~isempty(ips)
        for jj = 1:size(ips,1)
            ...
            if abs(d/inv_depth(row,col) - 1) < 0.1 %&& prod(ismember([model(combs(j,1)).face, model(combs(j,2)).face],unique(seg(np1,np2))))
                count_bp = count_bp+1;
...
%         v = v * h;
        ...
        [np1,np2] = neighbor_pixels(row,col,h,w,4);
        if abs(d/inv_depth(row,col) - 1) < 0.1 %&& prod(ismember([model(combs(j,1)).face, model(combs(j,2)).face, model(combs(j,3)).face],unique(seg(np1,np2))))
            count_ip = count_ip+1;
